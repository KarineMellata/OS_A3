...
int setup_syscall_filters()
{
	...
	filter_set_status = seccomp_rule_add(
                                            seccomp_ctx,                    // the context to which $
                                            SCMP_FAIL,                  // action to take on rule ma$
                                            SCMP_SYS(clone),              // get the sys_call numb$
                                            1,                              // any additional argume$
                                            SCMP_A0(SCMP_CMP_MASKED_EQ, CLONE_NEWUSER, CLONE_NEWUSER)
                                            );
        if (filter_set_status) {
            if (seccomp_ctx)
	...
	filter_set_status = seccomp_rule_add(
                                            seccomp_ctx,                    // the context to which $
                                            SCMP_FAIL,                  // action to take on rule ma$
                                            SCMP_SYS(chmod),              // get the sys_call numb$
                                            1,                              // any additional argume$
        ...
        if (filter_set_status) {
            if (seccomp_ctx)
                seccomp_release(seccomp_ctx);
            fprintf(stderr, "seccomp could not add KILL rule for 'chmod' S_ISUID: %m\n");
            return EXIT_FAILURE;
        }
	...
	filter_set_status = seccomp_rule_add(
        ...
        if (filter_set_status) {
            if (seccomp_ctx)
                seccomp_release(seccomp_ctx);
            fprintf(stderr, "seccomp could not add KILL rule for 'chmod' S_ISGID: %m\n");
            return EXIT_FAILURE;
        }
	/**
